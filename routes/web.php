...
Route::group(['middleware' => ['auth', 'verified', 'active']], function () {
    ...
    Route::get('get-handyman-commission/{id}', [HandymanController::class, 'getCommission'])->name('handyman.commission');
...
Route::get('/test-mail', function () {
    try {
        \Illuminate\Support\Facades\Mail::raw('This is a test email to verify SMTP configuration.', function ($message) {
            $message->to(' ben.ghezaiel@web.de')
                    ->subject('SMTP Test Email  Allah help us');
        });
        return 'Test email sent successfully to  we are confuesed allah help us  ben.ghezaiel@web.de';
    } catch (\Exception $e) {
