 ...
 */
function ugc_reason_options_for_js(): array
{
    $keys = [
    ...
    $labels = \Illuminate\Support\Facades\Lang::get('messages.profile_report_reasons', [], app()->getLocale());
    if (! is_array($labels)) {
        $labels = [];
    }
    ...
    return array_values(array_map(function ($key) use ($labels) {
        return [
            'value' => $key,
            'label' => $labels[$key] ?? \Illuminate\Support\Str::title(str_replace('_', ' ', $key)),
        ];
    }, $keys));
}
