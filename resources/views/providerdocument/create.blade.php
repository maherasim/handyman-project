                    ...
                    <div class="card-body p-0">
                        <div class="d-flex justify-content-between align-items-center p-3 flex-wrap gap-3">
                            ...
                            @if ($auth_user->can('providerdocument list'))
                                <a href="{{ route('providerdocument.show', ['providerdocument' => $providerdata->id]) }}"
                                    class=" float-end btn btn-sm btn-primary"><i class="fa fa-angle-double-left"></i>
                                    {{ __('messages.back') }}</a>
                            @endif
                        </div>
                    </div>
                        ...
                        <div class="row">
                            @if (auth()->user()->hasAnyRole(['admin', 'demo_admin']))
                                <div class="form-group col-md-4">
                                    {{ html()->label(
                                            __('messages.select_name', ['select' => __('messages.providers')]) . ' <span class="text-danger">*</span>',
                                            'provider_id',
                                        )->class('form-control-label') }}
                                    <br />
                                    {{ html()->select('provider_id', [$providerdata->id => $providerdata->display_name], $providerdata->id)->class('select2js form-group providers')->required()->attribute('data-placeholder', __('messages.select_name', ['select' => __('messages.providers')])) }}
                                </div>
                            @endif
                            ...
                            <div class="form-group col-md-4">
                                {{ html()->label(
                                        __('messages.select_name', ['select' => __('messages.document')]) . ' <span class="text-danger">* </span>',
                                        'document_id',
                                    )->class('form-control-label') }}
                                <br />
                                {{ html()->select(
                                        ...
                                        [
                                            optional($provider_document->document)->id =>
                                                optional($provider_document->document)->name . ' ' . $is_required,
                                        ],
                                        optional($provider_document->document)->id,
                            ...
                            @if (auth()->user()->hasAnyRole(['admin', 'demo_admin']))
                                <div class="form-group col-md-4">
                                    {{ html()->label(trans('messages.is_verify') . ' <span class="text-danger">*</span>', 'is_verified')->class('form-control-label') }}
                                    {{ html()->select('is_verified', ['1' => __('messages.verified'), '0' => __('messages.unverified')], old('is_verified'))->id('is_verified')->class('form-control select2js')->required() }}
                                </div>
                            @endif
                            ...
                            <div class="form-group col-md-4">
                                ...
                                <div class="custom-file">
                                    ...
                                    @if ($provider_document && getMediaFileExit($provider_document, 'provider_document'))
                                        <label
                                            class="custom-file-label upload-label">{{ $provider_document->getFirstMedia('provider_document')->file_name }}</label>
                                    @else
                                        <label
                                            class="custom-file-label upload-label">{{ __('messages.choose_file', ['file' => __('messages.document')]) }}</label>
                                    @endif
                                </div>
                            </div>
                            ...
                            @if (getMediaFileExit($provider_document, 'provider_document'))
                                <div class="col-md-2 mb-2 position-relative">
                                    ...
                                    @endphp
                                    @if ($extention)
                                        <img id="provider_document_preview" src="{{ $image }}" alt="#"
                                            class="attachment-image mt-1">
                                    @else
                                        <img id="provider_document_preview" src="{{ asset('images/file.png') }}"
                                            class="attachment-file">
                                    @endif
                                    <a class="text-danger remove-file"
                                        href="{{ route('remove.file', ['id' => $provider_document->id, 'type' => 'provider_document']) }}"
                                    ...
                                    <a href="{{ $image }}" class="d-block mt-2" download target="_blank"><i
                                            class="fas fa-download "></i> {{ __('messages.download') }}</a>
                                </div>
                            @endif
                        </div>
