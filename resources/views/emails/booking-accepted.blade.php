...
<head>
    ...
    <title>{{ __('messages.email_booking_accepted_title') }}</title>
        @include('emails._email_styles')
</head>
<body>
    <div class="email-container">
        <div class="header">
            <h1>{{ __('messages.email_booking_accepted_title') }}</h1>
        </div>

        <div class="content">
            <h2>{{ __('messages.email_dear_name', ['name' => $customer->display_name ?? $customer->first_name ?? __('messages.email_valued_user')]) }}</h2>

            <p>{{ __('messages.email_booking_accepted_intro') }}</p>

            <div class="success-box">
                <div style="font-size: 48px; margin-bottom: 10px;">🎉</div>
                <div class="message">{{ __('messages.email_booking_confirmed') }}</div>
            </div>
            ...
            <div class="info-box">
                ...
                <div class="detail-row">
                    ...
                    <span class="detail-value">
                        <span class="status-badge status-accepted">{{ $booking->status ? booking_detail_status_label($booking->status) : __('messages.booking_status_option_accept') }}</span>
                    </span>
                </div>
                <div class="detail-row">
            ...
            <div class="provider-info">
                ...
                <div class="detail-row">
                    ...
                    <span class="detail-value">{{ $provider->display_name ?? trim(($provider->first_name ?? '') . ' ' . ($provider->last_name ?? '')) ?: __('messages.not_available') }}</span>
                </div>
            ...
            <div class="action-box">
                <h3>{{ __('messages.email_booking_accepted_next_steps') }}</h3>
                <ul style="margin: 10px 0; padding-left: 20px; color: #155724;">
                    <li>{{ __('messages.email_booking_accepted_step_contact') }}</li>
                    <li>{{ __('messages.email_booking_accepted_step_available') }}</li>
                    <li>{{ __('messages.email_booking_accepted_step_prepare') }}</li>
                    <li>{{ __('messages.email_booking_accepted_step_reschedule') }}</li>
                </ul>
            </div>

            <div class="button-container">
                <a href="{{ route('booking.detail', $booking->id) }}" class="button">{{ __('messages.email_view_booking_details') }}</a>
            </div>

            <p style="margin-top: 30px; color: #6c757d; font-size: 14px;">
                <strong>{{ __('messages.email_note') }}:</strong> {{ __('messages.email_booking_accepted_manage_note') }}
            </p>
        </div>
