...
<head>
    ...
    <title>{{ __('messages.email_payment_split_set_title') }}</title>
    <style>
        body {
...
<body>
    <div class="email-container">
        <div class="header">
            <h1>{{ __('messages.email_payment_split_set_title') }}</h1>
        </div>

        <div class="content">
            <h2>{{ __('messages.email_dear_name', ['name' => $customer->display_name ?? $customer->first_name ?? __('messages.email_valued_user')]) }}</h2>

            <p>{{ __('messages.email_payment_split_set_intro', ['title' => optional($postJob)->title ?? '']) }}</p>

            <div class="success-box">
                <div style="font-size: 48px; margin-bottom: 10px;">💰</div>
                <div class="message">{{ (int)($bid->advance_percent ?? 0) }}% / {{ (int)($bid->remaining_percent ?? 0) }}%</div>
            </div>
            ...
            <div class="info-box">
                ...
                <div class="detail-row">
                    <span class="detail-label">{{ __('messages.email_payment_split_advance') }}:</span>
                    <span class="detail-value">{{ (int)($bid->advance_percent ?? 0) }}%</span>
                </div>
                <div class="detail-row">
                    <span class="detail-label">{{ __('messages.email_payment_split_remaining') }}:</span>
                    <span class="detail-value">{{ (int)($bid->remaining_percent ?? 0) }}%</span>
                </div>
            </div>
            ...
            <div class="action-box">
                ...
                <ul style="margin: 10px 0; padding-left: 20px; color: #155724;">
                    <li>{{ __('messages.email_payment_split_step_review') }}</li>
                    <li>{{ __('messages.email_payment_split_step_pay') }}</li>
                </ul>
            </div>
