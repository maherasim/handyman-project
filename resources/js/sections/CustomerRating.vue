<template>
    <a v-if="!customerrating || customerrating === ''"  href="javascript:void(0);" class="d-inline-block text-capitalize fw-bold mt-2"
        @click="openModal">{{$t('landingpage.rate_customer')}}</a>

    <a v-if="customerrating && customerrating.id"  href="javascript:void(0);" class="d-inline-block text-capitalize fw-bold mt-2"
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title text-capitalize" :id="modalLabelId">{{$t('landingpage.rate_customer')}}</h5>
...
// Create unique modal ID based on customer_id
const modalId = computed(() => `customerRatingModal_${props.customer_id}`)
const modalLabelId = computed(() => `customerRatingModalLabel_${props.customer_id}`)
...
const formSubmit = handleSubmit(async(values) => { 
    ...
    values.provider_id = props.provider_id;

    if(props.customerrating && props.customerrating.id){
        values.id = props.customerrating.id
    }

    try{ 
        const response = await fetch(STORE_CUSTOMER_RATING_API, {
            method: 'POST',
            headers: {
...
const openModal = () => {
    ...
    if (modalElement) {
        ...
        if (typeof bootstrap !== 'undefined' && bootstrap.Modal) {
            ...
            backdrop.id = `backdrop_${props.customer_id}`;
...
const editRating = () =>{
    ratingval.value = props.customerrating.rating
    rating.value = props.customerrating.rating
    review.value = props.customerrating.review
    const modalElement = document.getElementById(modalId.value);
    if (modalElement) {
        // Try Bootstrap 5 first, fallback to jQuery if available
        if (typeof bootstrap !== 'undefined' && bootstrap.Modal) {
...
const closeModal=()=>{
    ...
    if (modalElement) {
        ...
        if (typeof bootstrap !== 'undefined' && bootstrap.Modal) {
            ...
            const backdrop = document.getElementById(`backdrop_${props.customer_id}`);
            if (backdrop) {
                backdrop.remove();
            }
        }
